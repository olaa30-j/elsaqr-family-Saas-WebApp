...
const AdGrid = ({ ads, currentIndex }: IAdGrid) => {
    if (ads.length === 1) {
        return (
            <div className="flex h-full transition-all duration-500 gap-3 justify-center">
                <div className="md:w-3/4 w-full h-full shrink-0 px-2">
                    <AdCard ad={ads[0]} size="large" />
                </div>
            </div>
        );
    }
